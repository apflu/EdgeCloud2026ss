...
import { speakRobotLine } from '../lib/ttsClient';
...
export function RobotPanel({
  ...
  onLogSpeech: (tag: 'BOT_SUGGESTION' | 'CUSTOM_MESSAGE', text: string, ok: boolean, error?: string) => void;
  onPatientOk: () => void;
  onNoResponse: () => void;
}) {
  ...
  const [speakingCode, setSpeakingCode] = useState<RobotCommandCode | null>(null);
  const [speakError, setSpeakError] = useState<string | null>(null);

  async function handleSpeak(code: RobotCommandCode, speech: string) {
    setSpeakError(null);
    setSpeakingCode(code);
    try {
      await speakRobotLine(speech);
      onLogSpeech('BOT_SUGGESTION', speech, true);
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Failed to play robot speech.';
      setSpeakError(message);
      onLogSpeech('BOT_SUGGESTION', speech, false, message);
    } finally {
  ...
  async function handleSpeakCustom() {
    const text = customMessage.trim();
    if (!text) return;
    setCustomError(null);
    setCustomSpeaking(true);
    try {
      await speakRobotLine(text);
      onLogSpeech('CUSTOM_MESSAGE', text, true);
      setCustomMessage('');
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Failed to play custom message.';
      setCustomError(message);
      onLogSpeech('CUSTOM_MESSAGE', text, false, message);
    } finally {
      setCustomSpeaking(false);
    }
  }
      ...
      {options.length > 0 && activeOption ? (
        <div className='robot-box'>
          ...
          <div className='robot-option-grid'>
            {options.map((option) => (
              <div key={option.code} className='robot-option-row'>
                <button
                  type='button'
                ...
                <button
                  ...
                  title={`Play robot speech: "${option.speech}"`}
                  aria-label={`Play robot speech for ${option.label}`}
                  disabled={speakingCode === option.code}
                  onClick={() => handleSpeak(option.code, option.speech)}
                >
                  {speakingCode === option.code ? '…' : '🔊'}
                </button>
              </div>
            ))}
          </div>
          {speakError ? <p className='speak-error'>{speakError}</p> : null}
          ...
          <div className='custom-message-box'>
            ...
            <div className='custom-message-row'>
              <input
                ...
                onKeyDown={(event) => {
                  if (event.key === 'Enter') handleSpeakCustom();
                }}
                placeholder='Type a message for the robot to speak…'
                maxLength={500}
                disabled={customSpeaking}
                aria-label='Custom robot speech message'
              />
              <button
                type='button'
                className='speak-button'
                disabled={customSpeaking || customMessage.trim().length === 0}
                onClick={handleSpeakCustom}
              >
                {customSpeaking ? '…' : 'Speak'}
              </button>
            </div>
            {customError ? <p className='speak-error'>{customError}</p> : null}
          </div>
