...
export function useDashboardStream(intervalMs = 3000) {
  ...
  function logSpeechEvent(tag: 'BOT_SUGGESTION' | 'CUSTOM_MESSAGE', text: string, ok: boolean, error?: string) {
    setData((previous) => ({
      ...previous,
      audit: [
        {
          timestamp: now(),
          type: ok ? 'SPEECH_PLAYED' : 'SPEECH_FAILED',
          message: ok
            ? `[${tag}] Spoke: "${text}"`
            : `[${tag}] Failed to speak "${text}": ${error ?? 'unknown error'}`,
        },
        ...previous.audit.slice(0, 14),
      ],
    }));
